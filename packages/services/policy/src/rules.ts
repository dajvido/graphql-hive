...
function isRelevantCategory(cateogry?: CategoryType | CategoryType[]): boolean {
  if (!cateogry) {
    return false;
  }

  return Array.isArray(cateogry) ? cateogry.includes('Schema') : cateogry === 'Schema';
}
  ...
  switch (ruleName) {
    ...
    case 'naming-convention': {
      ...
      ruleDef.meta.schema.items.patternProperties = {
        '^(Argument|DirectiveDefinition|EnumTypeDefinition|EnumValueDefinition|FieldDefinition|FragmentDefinition|InputObjectTypeDefinition|InputValueDefinition|InterfaceTypeDefinition|ObjectTypeDefinition|ScalarTypeDefinition|UnionTypeDefinition)(.+)?$':
          {
            oneOf: [
              ruleDef.meta.schema.definitions.asObject,
              ruleDef.meta.schema.definitions.asString,
